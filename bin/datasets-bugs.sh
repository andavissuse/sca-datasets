...
[ -r "$confFile" ] && source ${confFile}
if [ -z "$confFile" ]; then
        echo "*** ERROR: No sca-databuild.conf file, exiting..." >&2
        exit 1
fi
...
[ $DEBUG ] && echo "*** DEBUG: $0: databuildTmpPath: $databuildTmpPath, bscScsDir: $bscScsDir, l3ScsDir: $l3ScsDir, datasetsPath: $datasetsPath" >&2

# create list of new supportconfigs that could be related to bugs
scsFile="$databuildTmpPath/new-bug-scs.txt"
cat "$databuildTmpPath/new-scs.txt" | grep -E "$bscScsDir|$l3ScsDir" > $scsFile
if [ ! -s "$scsFile" ]; then
